...
import { FadeIn, SquishBounce, TypingHeading, BorderGlow } from '../ui';
import { services } from '../../data/services';

export const ServicesSection = () => {
  return (
    <section id="skills" className="bg-[#121316] border-t border-white/5 rounded-t-[40px] sm:rounded-t-[50px] md:rounded-t-[60px] -mt-10 sm:-mt-12 md:-mt-14 px-5 sm:px-8 md:px-10 pt-24 pb-32 sm:pt-32 sm:pb-40 relative z-10 overflow-hidden">
        ...
        {services.map((svc, i) => (
          <FadeIn key={svc.num} delay={i * 0.1} y={30} className="w-full md:w-[calc(50%-1rem)] lg:w-[calc(33.333%-1.35rem)] flex-shrink-0">
            <BorderGlow
              ...
              className="w-full h-full shadow-xl hover:-translate-y-1 sm:hover:-translate-y-2 hover:scale-[1.02] transition-all duration-300 group"
            >
              <div className="flex flex-col w-full h-full p-6 sm:p-8 relative z-10">
                <div className="absolute -right-8 -top-8 w-32 h-32 bg-[#D7E2EA]/5 rounded-full blur-3xl group-hover:bg-[#D7E2EA]/10 transition-colors duration-500 pointer-events-none"></div>
                ...
                <div className="flex flex-wrap gap-2 z-10">
                  {svc.skills.map(skill => (
                    <a 
                      ...
                      target="_blank"
                      rel="noreferrer"
                      data-cursor-padding="4"
                      className="cursor-target px-3 py-1.5 bg-[#D7E2EA]/5 hover:bg-[#D7E2EA]/15 border border-[#D7E2EA]/10 rounded-full text-xs sm:text-sm text-[#D7E2EA]/80 font-medium transition-colors cursor-pointer"
                    >
                      {skill.name}
                    </a>
                  ))}
